...
describe("diffScans partitioning", () => {
  const a = scan(
    "before",
    [file("keep.txt", 10), file("gone.txt", 4), file("src/app.ts", 100)],
    ["src", "old"],
  );
  const b = scan(
    "after",
    [file("keep.txt", 10), file("new.txt", 7), file("src/app.ts", 120)],
    ["src", "vendor"],
  );
...
describe("formatReport", () => {
  const a = scan(
    "before",
    [file("gone.txt", 4), file("src/app.ts", 100), file("src/util.ts", 8)],
    ["src"],
  );
  const b = scan(
    "after",
