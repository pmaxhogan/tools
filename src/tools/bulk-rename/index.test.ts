...
describe('planRenames: template', () => {
  ...
  it('keeps a per extension counter separate from {n}', () => {
    const result = plan(
      scanOf([file('a.jpg'), file('b.png'), file('c.jpg'), file('d.png')]),
      { mode: 'template', template: '{ext}-{counter}-of-{n}', seqPad: 1 },
    );
    expect(opLines(result)).toEqual([
...
describe('planRenames: sequence mode', () => {
  ...
  it('numbers by size when asked', () => {
    const result = plan(
      scanOf([file('big.bin', { size: 900 }), file('small.bin', { size: 10 })]),
      { mode: 'sequence', prefix: 'f', seqPad: 1, sortBy: 'size' },
    );
...
describe('planRenames: clean mode', () => {
  it('strips diacritics, tidies spaces and drops awkward characters', () => {
    ...
    expect(opLines(result)).toEqual([
      'Café Ünïcode  (final)!.TXT -> cafe-unicode-final.TXT',
    ]);
  });
});
...
describe('planRenames: collisions', () => {
  ...
  it('orders a shifting run so every target is free when it runs', () => {
    const result = plan(
      scanOf([file('img-001.jpg'), file('img-002.jpg'), file('img-003.jpg')]),
      { mode: 'sequence', prefix: 'img-', seqStart: 0, seqPad: 3 },
    );
    expect(result.collisions).toEqual([]);
    expect(opLines(result)).toEqual([
