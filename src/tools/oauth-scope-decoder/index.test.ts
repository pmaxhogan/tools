...
function jwt(payload: Record<string, unknown>, header: Record<string, unknown> = { alg: 'RS256', typ: 'JWT' }): string {
...
describe('oauth-scope-decoder catalog', () => {
  ...
  it('never uses em or en dashes in prose', () => {
    ...
    const offenders = SCOPES.filter((s) =>
      /[–—]/.test(`${s.plainEnglish}${s.riskWhy}${s.label}`),
    );
...
describe('oauth-scope-decoder input extraction', () => {
  ...
  it('throws when a JWT carries no scope claim', () => {
    expect(() => extractScopes(jwt({ sub: 'abc', aud: 'api' }))).toThrowError(/scope, scp or scopes/);
  });
