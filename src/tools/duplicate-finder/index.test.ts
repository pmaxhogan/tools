...
describe('planHashing', () => {
  ...
  it('separates zero byte files from files the scan could not read', () => {
    ...
    expect(emptyFileGroup(scan)?.files.map((e) => e.path)).toEqual([
      'a/empty.txt',
      'b/empty.txt',
    ]);
  });
...
describe('groupByHash', () => {
  ...
  it('never invents a duplicate from the same file hashed twice', () => {
    ...
    expect(groupByHash([{ entry, hash: 'abcd' }, { entry, hash: 'abcd' }])).toEqual([]);
  });
...
describe('chooseKeeper', () => {
  ...
  it('refuses a strategy it does not have', () => {
    expect(() =>
      chooseKeeper(g, 'largest' as unknown as 'newest'),
    ).toThrowError(/not a way of choosing/);
  });
});
...
describe('chooseDeletions', () => {
  ...
  it('never proposes deleting the file it kept', () => {
    for (const strategy of ['first-alpha', 'shortest-path', 'newest', 'oldest', 'shallowest'] as const) {
