...
function tableRows(counts: [string, number][], indent: string): string[] {
  const lines: string[] = [];
  const width = Math.min(38, Math.max(...counts.slice(0, MAX_ROWS).map(([name]) => name.length), 1));
...
function describeBlueprint(bp: Obj, pad: string, lines: string[]): void {
  ...
  const icons = asArray(bp['icons']).map(iconLabel).filter((x): x is string => x !== null);
  ...
  lines.push(`${pad}Wires and circuit connections: ${wireParts.length ? wireParts.join('; ') : 'none'}`);
...
function describePlanner(kind: string, planner: Obj, pad: string, lines: string[]): void {
  ...
  lines.push(`${pad}This is a ${kind.replace(/_/g, ' ')}, not a blueprint, so it holds settings rather than entities.`);
}
...
function describeRoot(root: Obj, pad: string, lines: string[], heading = ''): void {
  ...
  if (book) {
    ...
    if (description) lines.push(`${pad}  Description: ${description.replace(/\s*\n\s*/g, ' / ')}`);
    const icons = asArray(book['icons']).map(iconLabel).filter((x): x is string => x !== null);
    lines.push(`${pad}  Icons: ${icons.length ? icons.join(', ') : 'none'}`);
  ...
  for (const kind of ['deconstruction_planner', 'upgrade_planner']) {
    ...
    lines.push(`${pad}${heading}${kind === 'upgrade_planner' ? 'Upgrade planner' : 'Deconstruction planner'}: ${label}`);
...
async function repairString(raw: string): Promise<RepairResult> {
  ...
  if (/[-_]/.test(text) && !/[+/]/.test(text)) {
    ...
    fixes.push('Converted URL-safe base64 characters ("-" and "_") back to the standard "+" and "/".');
  }
...
function stripReport(tally: StripTally, opts: FactorioOpts): string[] {
  ...
  if (!opts.stripTrees && !opts.stripRequests && !opts.stripTiles) {
    lines.push('No strip options were selected, so nothing was removed and the string was re-encoded as is.');
    return lines;
  }
  ...
  if (opts.stripTrees) {
    ...
    if (total === 0) {
      ...
      const detail = [...tally.removedEntities.entries()]
        ...
        .map(([name, n]) => `${name} x${n}`)
        .join(', ');
      lines.push(`Removed ${plural(total, 'environment entity', 'environment entities')}: ${detail}`);
    }
  }
  ...
  lines.push('Every remaining entity keeps its original entity_number, so wires and circuit connections still point at the right entities.');
  return lines;
}
