...
describe('factorio-blueprint-decoder: inspect', () => {
  ...
  it('counts entities by name, highest first', async () => {
    ...
    const order = ['transport-belt', 'inserter', 'fast-inserter'].map((n) => out.indexOf(`\n    ${n}`));
...
describe('factorio-blueprint-decoder: repair', () => {
    ...
    for (let salt = 0; salt < 500; salt++) {
      const candidate = encodeFixture({
        blueprint: {
          ...
          label: `Bus ${salt}`,
          version: VERSION_1_1_110,
          entities: Array.from({ length: 12 }, (_, i) => belt(i + 1, i, salt % 7, `entity-${i}-${salt}`)),
        },
      });
  ...
  it('fixes embedded newlines and a URL-encoded plus sign', async () => {
    ...
    expect(clean).toBe(await run(await run(good, { operation: 'json' }), { operation: 'reencode' }));
  });
  ...
  it('cuts trailing junk that is not part of the payload', async () => {
    const out = await run(`${encodeFixture(SIMPLE_BLUEPRINT)} <-- paste this!`, { operation: 'repair' });
  ...
  it('gives up honestly when characters are missing from the middle', async () => {
    await expect(run('this string is beyond saving', { operation: 'repair' })).rejects.toMatchObject({
      code: 'unrepairable',
    });
  });

  it('rejects empty repair input', async () => {
    await expect(run('   ', { operation: 'repair' })).rejects.toMatchObject({ code: 'empty-input' });
  });
});
  ...
  it('strips inside every blueprint of a book', async () => {
    const book = {
      blueprint_book: {
        ...
        label: 'Both',
        version: VERSION_1_1_110,
        blueprints: [{ index: 0, ...OVERGROWN }, { index: 1, ...OVERGROWN }],
      },
    };
...
describe('factorio-blueprint-decoder: errors', () => {
  ...
  it('rejects invalid JSON and empty input on re-encode', async () => {
    await expect(run('{oops}', { operation: 'reencode' })).rejects.toMatchObject({ code: 'invalid-json' });
...
describe('factorio-blueprint-decoder: helpers', () => {
  it('recognises environment entities without eating real machines', () => {
    for (const name of ['tree-01', 'dead-dry-hairy-tree', 'rock-huge', 'sand-rock-big', 'fish', 'dead-grey-trunk'])
