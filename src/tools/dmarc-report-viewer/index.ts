...
function gunzipOrFail(bytes: Uint8Array, what: string): Uint8Array {
  try {
    ...
    return fail(
      `${what} looks gzip compressed but could not be decompressed. It may be truncated.`,
    );
  }
}
...
function extractXmlFiles(input: Uint8Array | string): XmlFile[] {
  ...
  if (isZip(bytes)) {
    ...
    try {
      ...
      return fail(
        "The zip archive could not be opened. It may be truncated or password protected.",
      );
    }
    const files: XmlFile[] = [];
...
function groupRecords(records: DmarcRecord[], groupBySource: boolean): SourceGroup[] {
  ...
  return [...groups.values()].sort(
    (a, b) => b.count - a.count || a.sourceIp.localeCompare(b.sourceIp),
  );
}
...
export function run(input: Uint8Array | string, opts: DmarcOpts): string {
  ...
  if (first.sp) {
    ...
    lines.push(
      `  sp=${first.sp}: ${spEnglish ? spEnglish.split(": ")[1] : "subdomain policy"} (subdomains)`,
    );
  }
  ...
  if (first.adkim) {
    lines.push(
      `  adkim=${first.adkim}: DKIM alignment is ${ALIGNMENT_ENGLISH[first.adkim] ?? "unrecognized"}`,
    );
  }
  if (first.aspf) {
    lines.push(
      `  aspf=${first.aspf}: SPF alignment is ${ALIGNMENT_ENGLISH[first.aspf] ?? "unrecognized"}`,
    );
  }
  lines.push("");
  ...
  lines.push(
    groupBySource ? "SOURCES (grouped by IP, most mail first)" : "RECORDS (most mail first)",
  );
  lines.push(
    ...renderTable([
