...
describe('qr-code-scanner: decoding', () => {
  ...
  it('throws on non-integer or empty dimensions', () => {
    expect(() =>
      decodeQr({ data: new Uint8ClampedArray(4), width: 1.5, height: 1 }),
    ).toThrowError(/could not be read as pixels/);
    expect(() =>
      decodeQr({ data: new Uint8ClampedArray(0), width: 0, height: 0 }),
    ).toThrowError(/no width or height/);
  });
