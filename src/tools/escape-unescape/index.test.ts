...
describe("escape-unescape", () => {
  ...
  describe("java", () => {
    ...
    it("throws ToolError on a truncated \\u escape", () => {
      expect(() => run("\\u12", { format: "java", direction: "unescape" })).toThrowError(ToolError);
    });
  });
  ...
  describe("powershell", () => {
    it("wraps in single quotes and doubles embedded quotes (happy path)", () => {
      expect(run(`it's fine`, { format: "powershell", direction: "escape" })).toBe("'it''s fine'");
    });
  ...
  describe("ldap", () => {
    it("escapes * ( ) and \\ (happy path)", () => {
      expect(run("(a=b*)\\", { format: "ldap", direction: "escape" })).toBe("\\28a=b\\2a\\29\\5c");
    });
  ...
  describe("base32", () => {
    it("matches a known vector (happy path)", () => {
      expect(run("foobar", { format: "base32", direction: "escape" })).toBe("MZXW6YTBOI======");
    });
    ...
    it("throws ToolError on an invalid character", () => {
      expect(() => run("1", { format: "base32", direction: "unescape" })).toThrowError(ToolError);
    });
  });
  ...
  describe("base58", () => {
    ...
    it("throws ToolError on an excluded character like 0", () => {
      expect(() => run("0", { format: "base58", direction: "unescape" })).toThrowError(ToolError);
    });
  });
  ...
  describe("quoted-printable", () => {
    ...
    it("throws ToolError on a malformed escape", () => {
      expect(() => run("=zz", { format: "quoted-printable", direction: "unescape" })).toThrowError(
        ToolError,
      );
    });
  });
  ...
  describe("rot13", () => {
    it("matches the well-known vector", () => {
      expect(run("Hello, World!", { format: "rot13", direction: "escape" })).toBe("Uryyb, Jbeyq!");
    });
  ...
  describe("nato", () => {
    it("spells out a word (happy path)", () => {
      expect(run("SOS", { format: "nato", direction: "escape" })).toBe("Sierra Oscar Sierra");
    });
  ...
  describe("punycode", () => {
    ...
    it("leaves an ASCII-only domain untouched", () => {
      expect(run("example.com", { format: "punycode", direction: "escape" })).toBe("example.com");
    });
