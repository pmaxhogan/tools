...
describe('parsePageRanges', () => {
  it('parses a mixed spec into one group per comma segment', () => {
    expect(parsePageRanges('1-3,7,9-end', 12)).toEqual([
      [1, 2, 3],
      [7],
      [9, 10, 11, 12],
    ]);
...
describe('rotatePages', () => {
  ...
  it('rejects an angle that is not a quarter turn', async () => {
    await expect(
      rotatePages(await makeDoc(1), 'all', 45 as unknown as 90),
    ).rejects.toMatchObject({ code: 'invalid-rotation' });
  });
...
describe('watermarkPdf', () => {
  ...
  it('rejects an opacity outside 0 to 1', async () => {
    await expect(
      watermarkPdf(await makeDoc(1), { text: 'X', opacity: 4 }),
    ).rejects.toMatchObject({ code: 'invalid-opacity' });
    await expect(
      watermarkPdf(await makeDoc(1), { text: 'X', opacity: 0 }),
    ).rejects.toMatchObject({ code: 'invalid-opacity' });
  });

  it('rejects an impossible font size', async () => {
    await expect(
      watermarkPdf(await makeDoc(1), { text: 'X', fontSize: 0 }),
    ).rejects.toMatchObject({ code: 'invalid-font-size' });
  });
