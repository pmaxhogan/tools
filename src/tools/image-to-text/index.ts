 ...
 */
export function reconstructLayout(lines: OcrLine[] | null | undefined, pageWidth: number): string {
  const usable = (lines ?? []).filter((line) => (line?.text ?? "").trim().length > 0);
  if (usable.length === 0) return "";

  const charWidth = median(usable.map((line) => bboxWidth(line.bbox) / line.text.trim().length));
...
export function confidenceSummary(words: OcrWord[] | null | undefined): ConfidenceSummary {
  ...
  const lowConfidenceCount = list.filter((word) => (word.confidence ?? 0) < LOW_CONFIDENCE).length;

  const verdict: ConfidenceSummary["verdict"] = mean >= 85 ? "great" : mean >= 70 ? "good" : "poor";
