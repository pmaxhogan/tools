...
describe('collectLines and collectWords', () => {
  it('flattens the block, paragraph, line, word nesting in reading order', () => {
    expect(collectLines(samplePage()).map((l) => l.text)).toEqual([
      'Invoice 2026',
      'scanned copy',
    ]);
...
describe('confidenceSummary', () => {
  ...
  it('treats a word exactly at the threshold as acceptable', () => {
    expect(confidenceSummary([word('edge', LOW_CONFIDENCE, bbox(0, 0, 1, 1))]).lowConfidenceCount).toBe(
      0,
    );
  });
  ...
  it('advises on the photo rather than apologizing when nothing was found', () => {
    ...
    expect(summary).toMatchObject({ wordCount: 0, mean: 0, lowConfidenceCount: 0, verdict: 'poor' });
...
describe('formatResult', () => {
  it('renders plain text, blocks, and TSV from the same stored result', () => {
    ...
    expect(formatResult(page, { format: 'blocks', preserveLayout: false })).toContain('Block 1 of 2');
