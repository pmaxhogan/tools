...
describe("planCompression", () => {
  it("plans a 10 MB cap over 60 seconds with audio", () => {
    ...
    expect(planCompression({ targetBytes: 10_000_000, durationSec: 60, hasAudio: true })).toEqual({
      videoKbps: 1196,
      audioKbps: 96,
  ...
  it("plans a 50 MB cap over 10 minutes", () => {
    ...
    expect(planCompression({ targetBytes: 50_000_000, durationSec: 600, hasAudio: true })).toEqual({
      videoKbps: 550,
      audioKbps: 96,
  ...
  it("gives the whole budget to the picture when there is no audio", () => {
    ...
    expect(planCompression({ targetBytes: 10_000_000, durationSec: 60, hasAudio: false })).toEqual({
      videoKbps: 1292,
      audioKbps: 0,
      estimatedBytes: 9_690_000,
      feasible: true,
