 ...
 */
function makeGif(options: { width: number; height: number; delaysCs: number[] }): Uint8Array {
  const bytes: number[] = [];
...
describe("paletteWrap", () => {
  ...
  it("handles an empty filter as a pure re-encode", () => {
    expect(paletteWrap("")).toBe("[0:v]split[pgs][pgu];[pgs]palettegen[pal];[pgu][pal]paletteuse");
  });
...
describe("buildOptimize", () => {
  ...
  it("refuses lossy optimization, which ffmpeg cannot do", () => {
    const result = refusal(buildOptimize({ inputName: "a.gif", fps: 12, colors: 64, lossy: true }));
...
describe("buildSplit", () => {
  it("exports every frame with frame timing passed through", () => {
    ...
    expect(result.args).toEqual(["-i", "a.gif", "-vsync", "0", "-frames:v", "3", "out%04d.png"]);
