...
export function buildVcard(fields: VcardFields): string {
  ...
  if ((fields.phone ?? '').trim())
    out.push(`TEL;TYPE=cell:${escapeVcard(fields.phone!.trim())}`);
...
function normalisePhone(value: string, code: string, what: string): string {
  ...
  if (!raw)
    throw new ToolError(code, `The ${what} is required.`, 'Enter the number in full, including the country code.');
...
export function toIcalUtc(value: string | Date): string {
  ...
  if (value instanceof Date) {
    if (Number.isNaN(value.getTime()))
      throw new ToolError('bad-datetime', 'That date and time could not be read.', 'Pick the date again.');
    return format(value);
  }
