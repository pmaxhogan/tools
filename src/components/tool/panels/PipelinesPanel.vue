...
const firstNode = computed(() => (steps.value[0] ? NODE_BY_SLUG.get(steps.value[0].slug) : undefined));
...
const warnings = computed(() =>
  steps.value.length ? validatePipeline({ steps: steps.value, input: inputText.value }, metaFor) : [],
);
...
async function runNow(): Promise<void> {
  ...
  const def = { steps: steps.value.map((s) => ({ slug: s.slug, opts: { ...s.opts } })), input: inputText.value };
    ...
    <template v-else>
      ...
      >
        ...
        <div class="flex flex-col gap-1.5">
          ...
          >
            <optgroup
              v-for="group in pickerGroups(index)"
              :key="group.label"
              :label="group.label"
            >
              <option
        ...
        <template v-if="resultFor(index)">
          <div
            ...
            :class="isHintError(index) ? 'bg-secondary/60' : 'border-destructive/50 bg-destructive/5'"
          >
            <p :class="isHintError(index) ? 'font-medium text-muted-foreground' : 'font-medium text-destructive'">
              {{ resultFor(index)!.error!.message }}
            </p>
            <p
              v-if="resultFor(index)!.error!.fix"
      ...
      <details class="rounded-[10px] bg-secondary p-4 shadow-[var(--sh-inset)]">
        <summary class="cursor-pointer text-sm font-medium">
          What can and cannot be chained
        </summary>
