...
const visible = computed(() =>
  showAll.value ? filtered.value : filtered.value.slice(0, ROW_CAP),
);
...
const sensitiveLine = computed(() => {
  ...
  if (r.responseBodies > 0) {
    parts.push(
      `${r.responseBodies} saved response ${r.responseBodies === 1 ? 'body' : 'bodies'}`,
    );
  }
...
<template>
  <div class="flex flex-col gap-4 rounded-[18px] border bg-card p-5 shadow-[var(--sh-sm)] sm:p-6">
    ...
    <template v-if="model && summary">
        ...
        <div class="flex items-start gap-2">
          ...
          <div class="text-sm">
            ...
            <p class="mt-1 text-muted-foreground">
              {{ sensitive.entries }} of {{ summary.requests }} requests carry something a
              stranger could replay. The sanitized copy empties the cookie arrays, redacts the
              Cookie, Set-Cookie, Authorization and Proxy-Authorization headers, replaces request
              bodies with their size, redacts credential shaped query parameters, and drops saved
              response bodies.
            </p>
          </div>
        </div>
      ...
      <!-- Filters -->
      <div class="flex flex-wrap items-end gap-3 rounded-[10px] bg-secondary p-3 shadow-[var(--sh-inset)]">
            ...
            <tr class="text-left text-xs text-muted-foreground">
              ...
              >
                ...
                >
                  Start
                  <ArrowUp
                    v-if="sortKey === 'start' && sortDir === 'asc'"
                    class="size-3"
                  />
                  <ArrowDown
                    v-else-if="sortKey === 'start'"
                    class="size-3"
                  />
                </button>
              </th>
              <th
                scope="col"
                class="w-full px-3 py-2 font-medium"
              >
                Request
              </th>
              <th
              ...
              >
                ...
                >
                  Time
                  <ArrowUp
                    v-if="sortKey === 'duration' && sortDir === 'asc'"
                    class="size-3"
                  />
                  <ArrowDown
              ...
              <th
                scope="col"
                class="w-[320px] min-w-[320px] px-3 py-2 font-medium"
              >
                Waterfall
              </th>
            </tr>
                ...
                >
                  <div class="flex flex-col gap-3 text-xs">
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                      <div>
                        <div class="mb-1 font-semibold tracking-[0.04em] text-muted-foreground uppercase">
                          Request headers
                        </div>
                        <div
                        ...
                        >
                          ...
                          <span
                            ...
                            :class="hidden(rowId(entry), 'req', i, header.name) ? 'text-muted-foreground' : ''"
                          >
                          ...
                          >
                            <EyeOff
                              v-if="isRevealed(rowId(entry), 'req', i)"
                              class="size-3.5"
                            />
                            <Eye
                              v-else
                        ...
                        <p
                          v-if="entry.request.headers.length === 0"
                      ...
                      <div>
                        ...
                        >
                          ...
                          <span
                            class="min-w-0 flex-1 font-mono break-all"
                            :class="hidden(rowId(entry), 'res', i, header.name) ? 'text-muted-foreground' : ''"
                          >
                          ...
                          >
                            <EyeOff
                              v-if="isRevealed(rowId(entry), 'res', i)"
                              class="size-3.5"
                            />
                            <Eye
                              v-else
                        ...
                        <p
                          v-if="entry.response.headers.length === 0"
                          class="text-muted-foreground"
                        >
                          None recorded.
                        </p>
                    ...
                    <div
                      v-if="entry.request.queryString.length"
                      class="text-muted-foreground"
                    >
                      Query:
                      <span
