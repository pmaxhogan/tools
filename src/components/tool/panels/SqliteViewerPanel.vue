  ...
  try {
    ...
    sqlPanes.value = results.map((result, i) =>
      buildPane(
        result,
        `sql-${runSeq}-${i}`,
        results.length > 1 ? `Statement ${i + 1}` : 'Result',
      ),
    );
      ...
      <div class="grid gap-4 md:grid-cols-[minmax(180px,240px)_minmax(0,1fr)]">
        ...
        <div class="flex flex-col gap-3 rounded-[10px] bg-secondary p-3 shadow-[var(--sh-inset)]">
          ...
          <button
            ...
            :class="selected === table.name && !selectedIsView ? 'bg-card shadow-[var(--sh-sm)]' : ''"
          ...
          <template v-if="views.length">
            <div class="mt-1 text-xs font-semibold tracking-[0.04em] text-muted-foreground uppercase">
              Views
            </div>
            <button
              v-for="view in views"
              ...
              <thead>
                <tr class="text-left text-xs text-muted-foreground">
                  ...
                  >
                    ...
                    <span
                      v-if="column.pk"
                      class="ml-1 text-[var(--positive)]"
                      title="Primary key"
                    >key</span>
                    <span
                      v-if="column.type"
                      class="ml-1"
                    >{{ column.type.toLowerCase() }}</span>
                  </th>
                </tr>
              </thead>
