  ...
  >
    ...
    <template #controls="{ applyWrites }">
      <div class="flex flex-col gap-4">
        ...
        <p
          v-if="opts.mode === 'template'"
          class="text-xs text-muted-foreground"
        >
          Tokens: <code>{name}</code> the name without its extension,
          <code>{ext}</code> the extension, <code>{n}</code> a running number,
          <code>{counter}</code> a number that restarts for each extension,
          <code>{parent}</code> the containing folder, <code>{date}</code> the date the file was
          last changed.
        </p>
        <p
        ...
        >
          ...
          <div class="min-w-0">
            ...
            <ul class="mt-1 list-disc pl-4 text-xs break-words text-muted-foreground">
              <li
                v-for="(collision, i) in plan.collisions.slice(0, 5)"
                :key="i"
              >
                {{ collision }}
              </li>
        ...
        <div class="flex flex-col gap-2 rounded-[10px] bg-secondary p-3 shadow-[var(--sh-inset)]">
          ...
          <div
            ...
            class="max-h-96 overflow-auto rounded-[8px] bg-background"
          >
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-2 py-1.5 break-all">
                    <span :class="row.changed ? 'font-medium text-foreground' : 'text-muted-foreground'">
