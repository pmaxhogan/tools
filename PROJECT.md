...
1. Client-side by default. A server is permitted only when the job is _impossible_ in the browser.
...
- Do not use `localStorage` for user content, or the query string for shareable state.
  - Caveot: _do_ use localStorage for user settings (theme, bookmarks, control+k tool search)
